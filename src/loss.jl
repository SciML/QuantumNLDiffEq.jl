...
function calc_cost(DQC::DQCType, cost_params::CostParams)
	...
	return [[Scale(DQC.cost[i][j], cost_params.lambda[i][j]) for j in 1:length_range[i]] for i in 1:no_eqns]
end
...
function loss_diff(DQC::Vector{DQCType}, prob::AbstractODEProblem, M::AbstractVector, cost_params::AbstractCostParams, abh::AbstractBoundaryHandling, loss::Function, theta::Vector{Vector{Float64}})
	no_eqns = length(DQC)
...
function loss_diff(DQC::DQCType, prob::AbstractODEProblem, M::AbstractVector, cost_params::AbstractCostParams, abh::AbstractBoundaryHandling, loss::Function, theta::Vector{Float64})
...
function loss_reg(::Union{DQCType,Vector{DQCType}}, ::Vector{Float64}, ::NoRegularisation, ::AbstractCostParams, ::AbstractBoundaryHandling, ::Function, ::Any)
    return 0.0
end
...
function loss_bound(::Union{DQCType, Vector{DQCType}},  ::Real, ::Vector{Float64}, ::AbstractCostParams, ::Union{Floating, Optimized}, ::Function, ::Any)
	return 0.0
end

function loss_bound(DQC::DQCType,  M::Real, u0::Vector{Float64}, cost_params::AbstractCostParams, abh::Pinned, loss::Function, theta::Vector{Float64})
	no_eqns = length(u0)
	cost = calc_cost(DQC, cost_params)
...
function loss_bound(DQC::Vector{DQCType}, M::Real, u0::Vector{Float64}, cost_params::AbstractCostParams, abh::Pinned, loss::Function, theta::Vector{Vector{Float64}})
	no_eqns = length(u0)
	cost = calc_cost(DQC, cost_params)
	evalue = [calculate_evalue(DQC[i], cost[i], u0[i], abh, theta[i], M, M) for i in 1:no_eqns]
	loss_ind = abh.eta * sum([loss(evalue[i], u0[i]) for i in 1:no_eqns])
	return real(loss_ind)
end

function loss(DQC::Union{DQCType, Vector{DQCType}}, prob::AbstractODEProblem, config::DQCConfig, M::AbstractVector, theta)
	return loss_diff(DQC, prob, M, config.cost_params, config.abh, config.loss, theta) + loss_reg(DQC, prob.u0, config.reg, config.cost_params, config.abh, config.loss, theta) + loss_bound(DQC, M[1], prob.u0, config.cost_params, config.abh, config.loss, theta)
end